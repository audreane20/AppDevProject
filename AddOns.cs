...
namespace Menus
{
    ...
    public partial class AddOns : Form
    {
        ...
        private void AddOnsAddToCart_Click(object sender, EventArgs e)
        {
            ...
            var newItem = new Cart.CartItem
            {
                Name = ParentDrink.Name,          // includes size
                Quantity = DrinkQuantity,
                Price = ParentDrink.Price,
                Subtotal = ParentDrink.Price * DrinkQuantity,
                Category = ParentDrink.Category,
                AddOns = new List<Cart.AddOnItem>()
            };

            // Add-ons
            if (MilkQTY.Value > 0)
                newItem.AddOns.Add(new Cart.AddOnItem
                {
                    Name = "Milk 2%",
            ...
            // DUPLICATE CHECK & ADD/UPDATE CART
            Cart.AddOrUpdateCartItem(newItem);
