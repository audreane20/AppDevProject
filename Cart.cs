...
namespace Menus
{
    public partial class Cart : MenuSetup.MenuSetup
    {
       ...
       // string lang = System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName;
        ...
        private static bool AreSameAddOns(List<AddOnItem> a, List<AddOnItem> b)
        {
            if (a.Count != b.Count)
                return false;

            return a.All(x =>
                b.Any(y => y.Name == x.Name && y.Quantity == x.Quantity));
        }


        public static void AddOrUpdateCartItem(CartItem newItem)
        {
            var existingItem = CartItems.FirstOrDefault(item =>
                item.Name == newItem.Name &&
                item.Category == newItem.Category &&
                AreSameAddOns(item.AddOns, newItem.AddOns)
            );
